...
import { useActionState } from 'react'   //it handle the form action
import { NavLink, useNavigate } from 'react-router-dom';

async function  loginAction(_, formData) {
    const json= Object.fromEntries(formData);     //convert our formdata into json format
    const res = await fetch('http://127.0.0.1:8000/login/', {
        method: "POST",
        headers:{
            "Content-type": "application/json"
        },
        body: JSON.stringify(json)   //convert our req_body into json string
    });
    const data = await res.json()  //convert our response into json format

    if(res.ok){
        localStorage.setItem("userId",data.user_id);   //localStorage is used to store our id and name in browser after login (we cannot see in UI),
...
export default function LoginPage() {
    ...
    const [message, formAction, isPending]= useActionState(loginAction, "", {  // "" mean (initial state)empty string,after completing registerAction fn , which store response msg
        withPending : true
      });
