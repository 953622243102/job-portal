...
export default function ApplyJobPage() {
    ...
    const [result, formAction, isPending]= useActionState(applyJobAction, null, {  // "" mean (initial state)empty string,after applying job fn , which store response msg
        withPending : true
      });
      ...
      async function  applyJobAction(_, formData) {
        const res = await fetch('http://127.0.0.1:8000/apply/', {
            ...
            body: JSON.stringify({
                job:jobId,
                applicant:userId
            })   //convert our req_body into json string
        });
